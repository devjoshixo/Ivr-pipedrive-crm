...
| **Softphone placement** | **Companion Chrome extension** (softphone/click-to-dial) + Custom **Panel** (sidebar) | **UPDATE:** the softphone moved out of a Pipedrive Custom Floating Window into the IVR Solutions Chrome extension. Reason: Pipedrive's floating-window SDK `initialize()` never resolves while hidden, so it can't be reliably registered for **inbound** unless the user opens it first. The extension runs persistently in the browser (opens on startup), so it's always registered for inbound and handles `tel:` click-to-dial on any page. The Pipedrive app keeps OAuth, settings, the recording Panel, and call logging/sync. |
